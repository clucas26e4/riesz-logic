...
| prehmrr_ID : forall G T n r s, sum_vec r = sum_vec s -> preHMR (T :: G) -> preHMR ((vec s (MRS_covar n) ++ vec r (MRS_var n) ++ T) :: G)
| prehmrr_Z : forall G T r, preHMR (T :: G) -> preHMR ((vec r MRS_zero ++ T) :: G)
...
| prehmrr_one : forall G T r s, sum_vec s <= sum_vec r -> preHMR (T :: G) -> preHMR ((vec s MRS_coone ++ vec r MRS_one ++ T) :: G)
| prehmrr_diamond : forall T r s, sum_vec s <= sum_vec r -> preHMR ((vec s MRS_coone ++ vec r MRS_one ++ T) :: nil) -> preHMR ((vec s MRS_coone ++ vec r MRS_one ++ seq_diamond T) :: nil)
