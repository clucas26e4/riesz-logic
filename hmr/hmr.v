...
| hmrr_ID : forall G T n r s, sum_vec r = sum_vec s -> HMR P (T :: G) -> HMR P ((vec s (covar n) ++ vec r (var n) ++ T) :: G)
| hmrr_Z : forall G T r, HMR P (T :: G) -> HMR P ((vec r zero ++ T) :: G)
...
| hmrr_one : forall G T r s, sum_vec s <= sum_vec r -> HMR P (T :: G) -> HMR P ((vec s coone ++ vec r one ++ T) :: G)
| hmrr_diamond : forall T r s, sum_vec s <= sum_vec r -> HMR P ((vec s coone ++ vec r one ++ T) :: nil) -> HMR P ((vec s coone ++ vec r one ++ seq_diamond T) :: nil)
...
Proof.
  intros T pi.
  change (seq_diamond T) with (vec nil coone ++ vec nil one ++ seq_diamond T).
