...
Proof.
  ...
  - destruct L; try destruct p as [r1 T1]; inversion HeqG; subst.    
    ...
    esplit.
    Unshelve.
    2:{ apply hmrr_ex_seq with (vec (vec_mul_vec r1 s) (MRS_covar n) ++ vec (vec_mul_vec r1 r) (MRS_var n) ++ seq_mul_vec r1 T).
  ...
  - destruct L; try destruct p as [r1 T1]; inversion HeqG; subst.
    ...
    Unshelve.
    2:{ simpl.
        apply hmrr_ex_seq with (vec (vec_mul_vec r1 s) MRS_coone ++ vec (vec_mul_vec r1 r) MRS_one ++ seq_mul_vec r1 T).
        { rewrite <- ? seq_mul_vec_vec_mul_vec.
  ...
  - destruct L; [ | destruct L];  try destruct p as [r1 T1]; inversion HeqG; subst.
    destruct (IHpi ((r1, vec s MRS_coone ++ vec r MRS_one ++ T) :: nil) eq_refl) as [pi' Hpi'].
    esplit.
    Unshelve.
    2:{ simpl.
        apply hmrr_ex_seq with (vec (vec_mul_vec r1 s) MRS_coone ++ vec (vec_mul_vec r1 r) MRS_one ++ seq_diamond (seq_mul_vec r1 T)).
        { rewrite <- ? seq_mul_vec_vec_mul_vec.
          etransitivity ; [ | etransitivity ; [ apply Permutation_Type_app; [ reflexivity |  symmetry; apply seq_mul_vec_app_r ]| symmetry; apply seq_mul_vec_app_r ]].
...
Definition good_leaf_step_1 H D n :=
  fun G => {' (T', r , s ,t) : _ &
                               prod
                                 (G =  (vec t MRS_coone ++ vec s MRS_one ++ seq_mul_vec r D ++ seq_diamond T') :: H)
                                 (((sum_vec t <= sum_vec s)%R) *
                                  ({ pi : HMR_T ((vec t MRS_coone ++ vec s MRS_one ++ T') :: nil) & modal_depth_proof pi < n}))}.
...
Proof.
  ...
  - destruct L; try destruct p as [r1 T1]; inversion HeqG; subst; simpl.
    ...
    esplit.
    Unshelve.
    2:{ apply prehmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ T ++ seq_mul_vec r1 D) ; [ rewrite <- ? app_assoc; reflexivity | apply prehmrr_ID; try assumption]. }
    apply Hpi'.
  - destruct L; try destruct p as [r1 T1]; inversion HeqG; subst; simpl.
    destruct (IHpi ((r1, T) :: L) eq_refl) as [pi' Hpi'].
    esplit.
    Unshelve.
    2:{ apply prehmrr_ex_seq with (vec r MRS_zero ++ T ++ seq_mul_vec r1 D) ; [ rewrite <- ? app_assoc; reflexivity | apply prehmrr_Z; apply pi']. }
    apply Hpi'.
  - destruct L; try destruct p as [r1 T1]; inversion HeqG; subst; simpl.
    destruct (IHpi ((r1, vec r A ++ vec r B ++ T) :: L) eq_refl) as [pi' Hpi'].
    esplit.
  ...
  - destruct L; try destruct p as [r1 T1]; inversion HeqG; subst; simpl.
    ...
    esplit.
    Unshelve.
    2:{ apply prehmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ T ++ seq_mul_vec r1 D) ; [ rewrite <- ? app_assoc; reflexivity | apply prehmrr_one; try assumption]. }
    apply Hpi'.
  - destruct L ; [ | destruct L]; try destruct p as [r1 T1]; inversion HeqG; subst; simpl.
    assert ({pi' : preHMR ((vec s MRS_coone ++ vec r MRS_one ++ seq_mul_vec r1 D ++ seq_diamond T) :: H) &
...
Proof.
  ...
  subst.
  apply hmrr_ex_seq with (seq_mul_vec r D ++ seq_mul_vec (One :: nil) (vec t MRS_coone ++ vec s MRS_one ++ seq_diamond T1)).
  ...
  assert {L' & prod
                 (H = map (fun x => snd x) L')
                 (H = map (fun x => snd x ++ seq_mul_vec (fst x) (vec t MRS_coone ++ vec s MRS_one ++ seq_diamond T1)) L')} as [L' [Heq1 Heq2]].
  { clear; induction H.
    - split with nil; split; reflexivity.
    - destruct IHlist as [L [H1 H2]].
      split with ((nil, a) :: L).
  ...
  destruct (hmrr_M_step_1 ((One::nil, seq_mul_vec r D) :: L') nil (vec t MRS_coone ++ vec s MRS_one ++ seq_diamond T1) pi2' (hmrr_diamond _ _ _ _ Hle piG)) as [pi' Hleaves].
  rewrite <- app_nil_r.
  ...
  subst.
  apply hmrr_ex_seq with (vec (t' ++ vec_mul_vec r' t) MRS_coone ++ vec (s' ++ vec_mul_vec r' s) MRS_one ++ seq_diamond (seq_mul_vec r' T1 ++ D1)).
  { rewrite ? vec_app.
  ...
  { rewrite ? sum_vec_app.
    ...
    nra. }
  apply hmrr_ex_seq with (seq_mul_vec r' (vec t MRS_coone ++ vec s MRS_one ++ T1) ++ seq_mul_vec (One :: nil) (vec t' MRS_coone ++ vec s' MRS_one ++ D1)).
  { simpl; rewrite seq_mul_One.
    ...
    rewrite <- ? seq_mul_vec_vec_mul_vec.
    Permutation_Type_solve. }
  change  ((seq_mul_vec r' (vec t MRS_coone ++ vec s MRS_one ++ T1) ++ seq_mul_vec (One :: nil) (vec t' MRS_coone ++ vec s' MRS_one ++ D1)) :: nil)
    with
      (map (fun x => snd x ++ seq_mul_vec (fst x) (vec t' MRS_coone ++ vec s' MRS_one ++ D1)) ((One :: nil, seq_mul_vec r' (vec t MRS_coone ++ vec s MRS_one ++ T1)) :: nil)).
  assert { pi'' :  HMR_T (map (fun x : list Rpos * sequent => snd x) ((One :: nil,seq_mul_vec r' (vec t MRS_coone ++ vec s MRS_one ++ T1)) :: nil)) & modal_depth_proof pi'' < modal_depth_proof pi1} as [pi'' Hdiam''].
  { change (map (fun x : list Rpos * sequent => snd x)
                ((One :: nil, seq_mul_vec r' (vec t MRS_coone ++ vec s MRS_one ++ T1)) :: nil))
      with (map (fun x => seq_mul_vec (fst x) (snd x)) ((r' ,vec t MRS_coone ++ vec s MRS_one ++ T1) :: nil)).
    change ((vec t MRS_coone ++ vec s MRS_one ++ T1) :: nil) with (map (fun x => snd x) ((r' ,vec t MRS_coone ++ vec s MRS_one ++ T1) :: nil)) in piG.
    destruct (hmrr_mul_vec ((r', vec t MRS_coone ++ vec s MRS_one ++ T1) :: nil) piG) as [pi'' Hdiam''].
    split with pi''.
