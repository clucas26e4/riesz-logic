...
Proof.
  ...
  - destruct L; inversion Hperm; inversion Hsum; subst.
    ...
    apply hmrr_ex_seq with (vec s (MRS_covar n) ++ vec r (MRS_var n) ++ vec s1 (<S> (-S A)) ++ vec r1 (<S> A) ++ T); [ Permutation_Type_solve | ].
  ...
  - destruct L; inversion Hperm; inversion Hsum; subst.
    destruct p as [[s1 r1] T1]; simpl in *.
    apply hmrr_ex_seq with (vec r MRS_zero ++ vec s1 (<S> (-S A)) ++ vec r1 (<S> A) ++ T); [ Permutation_Type_solve | ].
    apply hmrr_Z.
    change ((vec s1 (<S> (-S A)) ++ vec r1 (<S> A) ++ T)
              :: map
              (fun x : list Rpos * list Rpos * list (Rpos * term) =>
  ...
  - destruct L; inversion Hperm; inversion Hsum; subst.
    destruct p as [[s1 r1] T1]; simpl in *.
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ vec s1 (<S> (-S A)) ++ vec r1 (<S> A) ++ T); [ Permutation_Type_solve | ].
  ...
  - destruct L; [ | destruct L]; inversion Hperm; try inversion X0.
    ...
    apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ seq_diamond (vec s' (-S A) ++ vec r' A ++ T)).
    { rewrite ? seq_diamond_app.
      rewrite ? vec_diamond.
      Permutation_Type_solve. }
    apply hmrr_diamond; try assumption.
    apply hmrr_ex_seq with (vec s' (-S A) ++ vec r' A ++ (vec s MRS_coone ++ vec r MRS_one ++ T)) ; [ Permutation_Type_solve | ].
...
Proof.
  ...
  induction A;intros G T r0 s Heq pi; unfold MRS_minus; fold MRS_minus.
  - apply hmrr_ID; assumption.
  - apply hmrr_ex_seq with (vec r0 (MRS_covar v) ++ vec s (MRS_var v) ++ T); [ Permutation_Type_solve | ].
  ...
  - apply hmrr_ex_seq with (vec r0 MRS_coone ++ vec s MRS_one ++ T) ; [ Permutation_Type_solve | ].
    apply hmrr_one; try nra; try assumption.
  - assert ({ L & prod
                  ( G = map (fun x => snd x) L)
...
Lemma hmrr_Z_can_inv P : forall G T r, HMR P ((vec r MRS_zero ++ T) :: G) -> HMR (hmr_frag_add_CAN P) (T :: G).
Proof.
  intros G T r pi.
  apply hmrr_can with MRS_zero r r; try reflexivity.
  apply hmrr_Z.
