...
Lemma only_diamond_seq_decomp :
  forall T,
    {' (r , s, D) : _ & prod
                       (Permutation_Type
                          (only_diamond_seq T)
                          (vec s MRS_coone ++ vec r MRS_one ++ D))
                       (sum_vec r - sum_vec s = sum_weight_one_seq T)}.
Proof.
  induction T; [split with (nil, nil, nil); split; try reflexivity; simpl; lra | ].
...
Lemma only_diamond_seq_decomp_no_diamond :
  forall T,
    ...
    {' (r , s) : _ & prod
                       (Permutation_Type
                          (only_diamond_seq T)
                          (vec s MRS_coone ++ vec r MRS_one))
                       (sum_vec r - sum_vec s = sum_weight_one_seq T)}.
Proof.
  induction T; intros Hnd; [split with (nil, nil); split; try reflexivity; simpl; lra | ].
  simpl in Hnd.
  destruct IHT as [[r s] [Hperm Heq]]; try (destruct a; simpl in * ;lia).
  destruct a as [a A].
  destruct A;
    try (esplit with (r, s); split; assumption);
    try (simpl modal_complexity_term in Hnd; exfalso; lia);
...
Proof.
  ...
  - intros Hb _; revert Hb.
    ...
    apply (lt_wf_rect n (fun n => forall T : sequent,
                             ...
                             0 <= sum_weight_one (T :: nil) ->
                             HMR P (flat_map only_diamond_seq (T :: nil) :: nil) ->
                             HMR P (T :: nil))).
    clear n.
    intros n IHn T Hlen Hat Hsum Hone Hstep.
    destruct (seq_basic_decomp_decr T Hat) as [[[[[n0 r] s] D] [Hr [Hperm Hdlen]]] | Hnil].
    + eapply hmrr_ex_seq ; [ symmetry; apply Hperm | ].
      ...
      * lia.
      * reflexivity.
      * apply seq_basic_perm with _ (vec s (MRS_covar n0) ++ vec r (MRS_var n0) ++ D) in Hat; try assumption.
      ...
      * intros n1.
        ...
        -- rewrite sum_weight_var_seq_vec_neq in Hsum; [ | intros H'; inversion H'; now auto | intros H'; inversion H'; now auto].
           rewrite sum_weight_var_seq_vec_neq in Hsum; [ | intros H'; inversion H'; now auto | intros H'; inversion H'; now auto].
           nra.
      * simpl in Hone; rewrite (sum_weight_one_seq_perm _ _ Hperm) in Hone.
        rewrite ? sum_weight_one_seq_app in Hone; rewrite ? sum_weight_seq_coone_app in Hone.
        simpl.
        rewrite ? sum_weight_one_seq_vec_neq in Hone; try now auto.
        nra.
      * simpl; simpl in Hstep.
        eapply hmrr_ex_seq; [ | apply Hstep].
        rewrite 2 app_nil_r.
        replace (only_diamond_seq D) with (only_diamond_seq (vec s (MRS_covar n0) ++ vec r (MRS_var n0) ++ D)) by (rewrite ? only_diamond_seq_app; rewrite only_diamond_seq_vec_covar; now rewrite only_diamond_seq_vec_var).
    ...
    + simpl in Hstep; rewrite app_nil_r in Hstep.
      ...
      apply hmrr_ex_seq with (vec s MRS_coone ++ vec r MRS_one ++ seq_diamond D); [ apply Permutation_Type_sym; apply Hperm | ].
      apply hmrr_diamond.
      { simpl in Hone.
        rewrite (sum_weight_one_seq_perm _ _ Hperm) in Hone.
        rewrite ? sum_weight_one_seq_app in Hone.
        rewrite sum_weight_one_seq_vec_one_eq in Hone; rewrite sum_weight_one_seq_vec_coone_eq in Hone.
        rewrite sum_weight_one_seq_seq_diamond in Hone.
        nra. }
      replace (vec s MRS_coone ++ vec r MRS_one ++ D) with (only_diamond_seq (vec s MRS_coone ++ vec r MRS_one ++ seq_diamond D)) by (rewrite ? only_diamond_seq_app; rewrite only_diamond_seq_vec_coone; rewrite only_diamond_seq_vec_one; now rewrite only_diamond_seq_only_diamond).
      eapply hmrr_ex_seq ; [ | apply Hstep].
      apply only_diamond_seq_perm; apply Hperm.
...
Fixpoint sum_weight_one_with_coeff G (L : list (option Rpos)) :=
  ...
  | T :: G , (Some r) :: L => ((projT1 r) * sum_weight_one_seq T) + sum_weight_one_with_coeff G L
  end.
...
Lemma sum_weight_one_with_coeff_atomic : forall G L,
    hseq_is_atomic G ->
    sum_weight_one_with_coeff G L = 0.
Proof.
  induction G; intros [ | r L] Hat; inversion Hat; simpl; try reflexivity.
  subst.
  ...
  forall G L,
    {' (r , s, T) : _ & prod
                       (Permutation_Type
                          ...
                          (vec s MRS_coone ++ vec r MRS_one ++ T))
                       (sum_vec r - sum_vec s = sum_weight_one_with_coeff G L) }.
Proof.
...
Lemma sum_weight_one_with_coeff_oadd_Rpos_list : forall G L1 L2,
    length L1 = length L2 ->
    sum_weight_one_with_coeff G (oadd_Rpos_list L1 L2) = sum_weight_one_with_coeff G L1 + sum_weight_one_with_coeff G L2.
Proof.
...
Lemma sum_weight_one_with_coeff_omul_Rpos_list : forall G L1 r,
    sum_weight_one_with_coeff G (map (mul_Rpos_oRpos r) L1) = (projT1 r) * sum_weight_one_with_coeff G L1.
Proof.
...
Lemma sum_weight_one_with_coeff_add_nat_list : forall G L1 L2,
    length L1 = length L2 ->
    sum_weight_one_with_coeff G (map nat_oRpos (add_nat_list L1 L2)) = sum_weight_one_with_coeff G (map nat_oRpos L1) + sum_weight_one_with_coeff G (map nat_oRpos L2).
Proof.
  intros G L1; revert G; induction L1; intros G L2 Hlen; destruct L2; try (now inversion Hlen); [ destruct G; simpl; nra | ].
  destruct G; [ simpl ; nra | ].
  inversion Hlen.
...
Lemma sum_weight_one_with_coeff_mul_nat_list : forall G L1 n',
    sum_weight_one_with_coeff G (map nat_oRpos (map (Nat.mul (S n')) L1)) = (INR (S n')) * sum_weight_one_with_coeff G (map nat_oRpos L1).
Proof.
    ...
    { L' &
      prod (Permutation_Type L L') 
           ((forall n, sum_weight_var_with_coeff n G L = sum_weight_var_with_coeff n H L') *
            (sum_weight_one_with_coeff G L = sum_weight_one_with_coeff H L') *
    ...
    { L' &
      prod (Permutation_Type L L') 
           ((forall n, sum_weight_var_with_coeff n G (map nat_oRpos L) = sum_weight_var_with_coeff n H (map nat_oRpos L')) *
            (sum_weight_one_with_coeff G (map nat_oRpos L) = sum_weight_one_with_coeff H (map nat_oRpos L')) *
...
Lemma sum_weight_one_with_coeff_all_0 :
  forall G L,
    Forall_inf (fun x => x = None) L ->
    sum_weight_one_with_coeff G L = 0.
Proof.
...
Lemma only_diamond_seq_no_diamond : forall T,
    modal_complexity_seq T = 0%nat ->
    {' (r, s) : _ & prod (Permutation_Type (only_diamond_seq T) (hseq.vec s MRS_coone ++ hseq.vec r MRS_one))
                          (hseq.sum_vec r - hseq.sum_vec s = sum_weight_one_seq T) }.
Proof.
  induction T; intros Heq.
  - split with (nil, nil).
    split; simpl; try reflexivity.
    lra.
  - destruct a as [a A].
    destruct A; simpl; try (apply IHT; simpl in Heq; lia);
      destruct IHT as [[r s] [Hperm Hone]]; simpl in Heq; try lia;
        try (exfalso; revert Heq; destruct (modal_complexity_seq T); intros Heq; lia).
    + split with (a :: r, s).
      split; [ simpl; Permutation_Type_solve | ].
      simpl; lra.
    + split with (r, a :: s).
      split; [ simpl; Permutation_Type_solve | ].
      simpl; lra.
...
Lemma concat_with_coeff_mul_only_diamond_no_diamond: forall G L,
    modal_complexity_hseq G = 0%nat ->
    {' (r, s) : _ & prod (Permutation_Type (concat_with_coeff_mul (only_diamond_hseq G) L) (hseq.vec s MRS_coone ++ hseq.vec r MRS_one))
                         (hseq.sum_vec r - hseq.sum_vec s = sum_weight_one_with_coeff G L) }.
Proof.
  induction G; intros L Hnd; destruct L; simpl;
    try (split with (nil, nil); split; [ reflexivity | lra]).
  specialize (IHG L) as [[r s] [Hperm Hone]].
  { simpl in Hnd; lia. }
  destruct o; [ | split with (r , s); split; simpl; assumption ].
  destruct (only_diamond_seq_no_diamond (hseq.seq_mul r0 a)) as [[r' s'] [Hperm' Hone']].
  { simpl in Hnd; rewrite hseq.modal_complexity_seq_mul; lia. }
  split with (r' ++ r , s' ++ s).
  split.
  - rewrite ? hseq.vec_app.
    rewrite only_diamond_seq_mul.
    Permutation_Type_solve.
  - rewrite ? hseq.sum_vec_app.
    rewrite sum_weight_one_seq_mul in Hone'.
    nra.
Qed.
