...
Proof.
  ...
  apply ex_seq with ((vec r (-S (A +S B)) ++ vec r A ++ vec r B) ++ (vec r (A +S B) ++ T)); [ perm_solve | ].
  apply M; [ | apply pi].
  apply ex_hseq with (G ++ ((vec r (-S (A +S B)) ++ vec r A ++ vec r B) :: nil)); [ perm_solve | ].
  apply W_gen.
  unfold minus; fold (-S A); fold (-S B).
  apply plus.
  apply ex_seq with (vec r (-S A) ++ vec r A ++ vec r (-S B) ++ vec r B); [ perm_solve | ].
...
Proof.
  ...
  apply ex_seq with ((vec r (-S (r0 *S A)) ++ vec (mul_vec r r0) A) ++ (vec r (r0 *S A) ++ T)); [ perm_solve | ].
  apply M; [ | apply pi ].
  apply ex_hseq with (G ++ ((vec r (-S (r0 *S A)) ++ vec (mul_vec r r0) A) :: nil)); [ perm_solve | ].
  apply W_gen.
...
Proof.
  move => G T A B r pi.
  apply can with (A \/S B) r r; try reflexivity.
  apply ex_seq with ( (vec r (-S (A \/S B)) ++ vec r B) ++ (vec r (A \/S B) ++ T)); [perm_solve | ].
  apply M.
  2:{ eapply ex_hseq; [ | apply W; apply pi ].
      apply perm_swap. }
  apply ex_hseq with ((vec r A ++ T) :: (vec r (-S (A \/S B)) ++ vec r B) :: G); [ apply perm_swap | ].
  apply can with (A \/S B) r r ; try reflexivity.
  apply ex_seq with ( (vec r (-S (A \/S B)) ++ vec r A) ++ (vec r (A \/S B) ++ T)); [perm_solve | ].
  apply M.
  2:{ eapply ex_hseq; [ | apply W; apply pi].
      apply perm_swap. }
  unfold minus; fold minus.
  apply min.
  - apply ex_hseq with (( (vec r (-S A /\S -S B) ++ vec r B) :: G) ++ ((vec r (-S A) ++ vec r A) :: nil)); [ perm_solve | ].
    apply W_gen.
    replace (vec r (-S A) ++ vec r A) with (vec r (-S A) ++ vec r A ++ nil) by (now rewrite app_nil_r).
    apply ID_gen; [reflexivity | apply INIT].
  - eapply ex_hseq ; [ apply perm_swap | ].
    apply min.
    + apply S.
      apply ex_seq with (vec r (-S A) ++ vec r A ++ vec r (-S B) ++ vec r B ++ nil) ; [ perm_solve | ].
      apply ID_gen; [ reflexivity | apply ID_gen ; [ reflexivity | ] ].
      apply ex_hseq with (G ++ (nil :: nil)); [ perm_solve | ].
...
Proof.
  ...
  unfold minus; fold minus.
  apply max.
  apply ex_hseq with (((vec r (-S B) ++ vec r A) :: G) ++ ((vec r (-S A) ++ vec r A) :: nil)); [ perm_solve | ].
  apply W_gen.
...
Proof.
  ...
  unfold minus; fold minus.
  apply max.
  apply ex_hseq with (((vec r (-S A) ++ vec r B) :: G) ++ ((vec r (-S B) ++ vec r B) :: nil)); [ perm_solve | ].
  apply W_gen.
  rewrite -(app_nil_r (vec r B)).
  apply ID_gen; [ reflexivity | apply INIT].
Qed.
