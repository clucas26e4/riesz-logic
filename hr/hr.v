...
| hrr_ID : forall G T n r s, sum_vec r = sum_vec s -> HR P (T :: G) -> HR P ((vec s (RS_covar n) ++ vec r (RS_var n) ++ T) :: G)
| hrr_Z : forall G T r, HR P (T :: G) -> HR P ((vec r RS_zero ++ T) :: G)
...
Lemma hrr_C_copy P : forall G T n,
    HR P (copy_seq (S n) T :: G) ->
    HR P (T :: G).
Proof.
...
Lemma hrr_C_copy_inv P : forall G T n,
    HR P (T :: G) ->
    HR (hr_frag_add_M P) (copy_seq n T :: G).
Proof.
  ...
  - simpl.
    specialize (IHn pi).
    apply hrr_M; try reflexivity; try assumption.
