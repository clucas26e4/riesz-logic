...
Proof.
  ...
  induction A;intros G T r0 s Heq pi; unfold RS_minus; fold RS_minus.
  - apply hrr_ID; assumption.
  - apply hrr_ex_seq with (vec r0 (RS_covar v) ++ vec s (RS_var v) ++ T); [ Permutation_Type_solve | ].
...
Lemma hrr_Z_can_inv P : forall G T r, HR P ((vec r RS_zero ++ T) :: G) -> HR (hr_frag_add_CAN P) (T :: G).
Proof.
  intros G T r pi.
  apply hrr_can with RS_zero r r; try reflexivity.
  apply hrr_Z.
...
Proof.
  ...
  - remember (s :: vs) as vs'; remember (r :: vr) as vr'.
    ...
    2:{ change (hr_frag_T_M) with (hr_frag_add_T hr_frag_T_M).
        apply hrr_T_vec_inv.
...
(** Additionnal rules that required hrr_ID_gen *)
Lemma hrr_can_fuse P :
  forall G T A r1 r2,
    HR P (((r1, A) :: (r2 , A) :: T) :: G) ->
    HR (hr_frag_add_M (hr_frag_add_CAN P)) (((plus_pos r1 r2, A) :: T) :: G).
Proof.
  intros G T A r1 r2 pi.
  apply hrr_can with A (r1 :: r2 :: nil) (r1 :: r2 :: nil); auto.
  apply hrr_ex_seq with (((vec (r1 :: r2 :: nil) (-S A) ++ vec (plus_pos r1 r2 :: nil) A)) ++ (vec (r1 :: r2 :: nil) A ++ T)) ; [Permutation_Type_solve | ] .
  apply hrr_M; auto.
  - apply hrr_ex_hseq with (G ++ (vec (r1 :: r2 :: nil) (-S A) ++ vec (plus_pos r1 r2 :: nil) A) :: nil); [ Permutation_Type_solve | ].
    apply hrr_W_gen.
    rewrite <- (app_nil_r (vec (r1 :: r2 :: nil) (-S A) ++ vec (plus_pos r1 r2 :: nil) A)); rewrite<- app_assoc;apply hrr_ID_gen ; [ | apply hrr_INIT].
...
Lemma hrr_can_unfuse P :
  forall G T A r1 r2,
    HR P (((plus_pos r1 r2, A) :: T) :: G) ->
    HR (hr_frag_add_M (hr_frag_add_CAN P)) (((r1, A) :: (r2 , A) :: T) :: G).
Proof.
  intros G T A r1 r2 pi.
  apply hrr_can with A (plus_pos r1 r2 :: nil) (plus_pos r1 r2 :: nil); auto.
  apply hrr_ex_seq with (((vec (plus_pos r1 r2 :: nil) (-S A) ++ vec (r1 :: r2 :: nil) A)) ++ (vec (plus_pos r1 r2 :: nil) A ++ T)) ; [Permutation_Type_solve | ] .
  apply hrr_M; auto.
  - apply hrr_ex_hseq with (G ++ (vec (plus_pos r1 r2 :: nil) (-S A) ++ vec (r1 :: r2 :: nil) A) :: nil); [ Permutation_Type_solve | ].
    apply hrr_W_gen.
    rewrite <- (app_nil_r (vec (plus_pos r1 r2 :: nil) (-S A) ++ vec (r1 :: r2 :: nil) A)); rewrite<- app_assoc;apply hrr_ID_gen ; [ | apply hrr_INIT].
    destruct r1; destruct r2; simpl; nra.
  - simpl.
    apply HR_le_frag with P; try assumption.
    destruct P; repeat split; Bool.destr_bool.
