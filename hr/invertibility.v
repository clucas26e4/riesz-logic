...
(map (fun x => (vec (fst x) RS_zero ++ snd x)) L) is thus the hypersequent (|- \vec{r_0}.0, T_0 | ... | |- \vec{r_n}.0, T_n) *)
Lemma hrr_Z_inv_gen P : forall L,
    HR P (map (fun x => (vec (fst x) RS_zero ++ snd x)) L) ->
    HR P (map (fun x => (snd x)) L).
Proof.
  intros L.
  remember (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) RS_zero ++ snd x) L) as G.
  assert (Allperm G (map (fun x : list Rpos * list (Rpos * term) => vec (fst x) RS_zero ++ snd x) L)) by (rewrite <- HeqG; clear; induction G; simpl; try now constructor).
  clear HeqG.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    apply hrr_ex_seq with (vec s (RS_covar n) ++ vec r (RS_var n) ++ Db).
    { Permutation_Type_solve. }
  ...
  - destruct L; inversion Hperm; subst.
    ...
    apply hrr_ex_seq with (vec c1 RS_zero ++ T').
    { Permutation_Type_solve. }
    apply hrr_Z.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (RS_zero <> A +S B) as Hneq by now auto.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (RS_zero <> r0 *S A) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
...
Proof.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hrr_ex_seq with (vec s (RS_covar n) ++ vec r (RS_var n) ++ vec r1 A ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hrr_ID; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (A +S B <> RS_zero) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hrr_ex_seq with (vec r RS_zero ++ vec r1 A ++ vec r1 B ++ Db).
    { etransitivity; [ apply Permutation_Type_app_comm | ].
      rewrite <- ? app_assoc; repeat (apply Permutation_Type_app; try reflexivity).
      Permutation_Type_solve. }
...
Proof with try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (a *S A <> (RS_covar n)) as Hnc by now auto.
    assert (a *S A <> (RS_var n)) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hrr_ex_seq with (vec s (RS_covar n) ++ vec r (RS_var n) ++ vec (mul_vec a r1) A  ++ Db).
    { Permutation_Type_solve. }
    apply hrr_ID; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    simpl.
    destruct p as [r1 T1]; simpl in *.
    assert (a *S A <> RS_zero) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hrr_ex_seq with (vec r RS_zero ++ vec (mul_vec a r1) A ++ Db).
    { etransitivity; [ apply Permutation_Type_app_comm | ].
      rewrite <- ? app_assoc; repeat (apply Permutation_Type_app; try reflexivity).
      Permutation_Type_solve. }
    apply hrr_Z; try assumption.
...
Proof.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    { destruct r1 ; [ | destruct r2 ].
      ...
      - apply hrr_C.
        ...
        apply hrr_C; change (hr_frag_T_M) with (hr_frag_add_T hr_frag_T_M); apply hrr_T_vec with (r2 :: r3); try now auto.
    ...
    { destruct r1 ; [ | destruct r2 ].
      ...
      - apply hrr_C.
        ...
        apply hrr_C; change (hr_frag_T_M) with (hr_frag_add_T hr_frag_T_M); apply hrr_T_vec with (r0 :: r1); try now auto.
        eapply hrr_ex_hseq ; [ apply Permutation_Type_skip; etransitivity ; [ apply Permutation_Type_swap | apply Permutation_Type_skip; apply Permutation_Type_swap ] | ].
        apply hrr_S.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (A \/S B <> (RS_var n)) as Hnv by now auto.
    destruct (perm_decomp_vec_neq_2 _ _ _ _ _ _ _ _ Hnc Hnv X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hrr_ex_seq with (vec s (RS_covar n) ++ vec r (RS_var n) ++ vec r1 A ++ Db).
    { Permutation_Type_solve. }
    apply hrr_ID; try assumption.
    eapply hrr_ex_hseq ; [ rewrite app_comm_cons; apply Permutation_Type_app_comm | ].
    simpl.
    apply hrr_ex_seq with (vec s (RS_covar n) ++ vec r (RS_var n) ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hrr_ID; try assumption.
  ...
  - destruct L; inversion Hperm; subst.
    ...
    assert (A \/S B <> RS_zero) as Hneq by now auto.
    destruct (perm_decomp_vec_neq _ _ _ _ _ _ Hneq X) as [ [[[Ta Tb] Da ] Db] [H1' [[[H2' H3'] H4'] H5']]].
    apply hrr_ex_seq with (vec r RS_zero ++ vec r1 A ++ Db).
    { Permutation_Type_solve. }
    apply hrr_Z; try assumption.
    eapply hrr_ex_hseq ; [ rewrite app_comm_cons; apply Permutation_Type_app_comm | ].
    simpl.
    apply hrr_ex_seq with (vec r RS_zero ++ vec r1 B ++ Db).
    { Permutation_Type_solve. }
    apply hrr_Z; try assumption.
    eapply hrr_ex_hseq; [ |  apply (IHpi ((r1 , Db) :: L))].
    { simpl.
...
Lemma hrr_Z_inv : forall G T r,  HR_T_M ((vec r RS_zero ++ T) :: G) -> HR_T_M (T :: G).
Proof.
  intros G.
  assert ({ L & prod
                  ( G = map (fun x : list Rpos * list (Rpos * term) => vec (fst x) RS_zero ++ snd x) L)
