...
Lemma IPP_eq_1 : forall (u : nat -> nat -> R) m,
    ...
    {' (v , i) : (nat -> nat -> R) * nat & prod (is_subseq v u)
                                             ((i < m)%nat *
                                              (forall n, v n i = 1))}.
Proof.
  intros u m H.
  ...
  destruct (IPP (fun n => u_n_ante_1 (fun i => u n i) m) (S m)) as [[v i] [Hsubseq [Hlt Hipp]]].
  - intros n.
    apply u_n_ante_1_lt.
    reflexivity.
  - destruct Hsubseq as [phi H'].
    split with (fun n => u (phi n), i).
...
Lemma seq_compact_Rn : forall n lb ub (u : nat -> list R),
    ...
    {' (v, ul) & prod (is_subseq v u)
                       ((length ul = n) *
                        (forall j, (j < n)%nat -> is_lim_seq (fun i => nth j (v i) 0) (nth j ul 0)))}.
Proof.
  enough (forall k n lb ub (u : nat -> list R),
             ...
             {' (v, ul) & prod (is_subseq v u)
                                 ((length ul = n) *
                                  (forall j, (j <= k)%nat -> is_lim_seq (fun i => nth j (v i) 0) (nth j ul 0)))}).
  { intros n lb ub u Hlen Hborned.
    destruct n.
    2:{ destruct (X n (S n) lb ub u (Nat.lt_succ_diag_r n) Hlen Hborned) as [[v ul] [Hsubseq [Hlenul Hlimul]]].
        split with (v, ul).
        repeat split; try assumption.
        intros j Hlt.
        apply Hlimul.
  ...
  - destruct (SequentialCompactness (fun n => nth 0 (u n) 0) lb ub) as [[v l] [Hsubseq Hlim]].
    { intros i.
      destruct (Hborned i 0%nat); try split; assumption. }
    destruct Hsubseq as [phi Hsubseq].
    split with (fun n => u (phi n), l :: repeat 0 (n - 1)).
    repeat split.
    + split with phi.
      destruct Hsubseq.
      split; auto.
    ...
    + intros j Hle.
      ...
      simpl.
      apply is_lim_seq_ext with v; destruct Hsubseq; assumption.
  - destruct (IHk n lb ub u) as [[v ul] [[phi Hsubseq] [Hlenul Hlimul]]]; try assumption; try lia.
    destruct (SequentialCompactness (fun n => nth (S k) (u (phi n)) 0) lb ub) as [[v' l] [[phi' Hsubseq'] Hlim]].
    { intros i.
      destruct (Hborned (phi i) (S k)); try split; assumption. }
    split with (fun i => u (phi (phi' i)), replace ul l (S k)).
    repeat split.
    + split with (fun i => phi (phi' i)).
      split; auto.
      intros i.
      ...
      * case_eq ((S (phi' i)) =? phi' (S i));
        ...
        -- apply Nat.lt_le_incl.
           apply subseq_support_implies_incr; [destruct Hsubseq; assumption | ].
           assert (phi' i < phi' (S i))%nat.
           { destruct Hsubseq'; apply s. }
           lia.
    ...
    + intros j Hle.
      ...
      * rewrite replace_nth_eq; try lia.
        ...
        destruct Hsubseq'; auto.
      * rewrite replace_nth_neq; try lia.
        apply is_lim_seq_subseq with (fun i => nth j (v i) 0).
        -- split with phi'.
           destruct Hsubseq'; destruct Hsubseq.
